...
## 9 sep 2026 | Product:warn | Platform:warn | Groei:ok
probleem: Dagronde langs de negen afdelingen. Zeven experiment-rijen liepen achter op de werkelijkheid (E13, E15, E17 en de design-pass stonden op "branch" terwijl PR #30, #32, #46 en #52 al gemerged en gedeployed waren); E18 stond twee keer in de tabel; de meetdata van E14 en E18 (8 sep) waren verstreken zonder effectmeting. De live totalen waren niet op te halen: `metrics.js` gaf "permission denied for table drivers" omdat de lokale SUPABASE_KEY een publishable key is en RLS sinds PR #39 aanstaat; de Supabase-MCP heeft geen access-token.
hypothese: De sessies die deployen schrijven hun eigen logboek-regel maar werken de experiment-rij niet bij; een dagronde die PR-status en site tegen de tabel legt vangt dat op. De blinde database is een gevolg van de security-gate, niet van een storing.
wijziging: Effectmeting E14: alle zes templates staan op approved in Twilio (script `submit-whatsapp-templates.js`, 9 sep), criterium van 8 sep gehaald, status ok; 1 okt blijft staan. Effectmeting E18 design-pass: live sinds 8 sep 20:50 (PR #52, CI 34277203085 test, gate en deploy groen; `curl zendiq.nl` toont DM Sans), audit 16/20 gehaald, critique 21/32 niet, whatsapp_click meten 4 okt. Schademelding hernummerd naar E19. E13, E15, E17 op live gezet met PR-nummer; E5 en E12 van "wacht op Meta" naar "SID's op Render". Besluiten 10 t/m 12 toegevoegd (database-toegang, Dependabot @anthropic-ai/sdk 0.88.0 naar 0.91.1, PR #51 en #45 wachten op review). Bewijs verder: health 200 met uptime 39085 s sinds de deploy van gisteravond; laatste drie CI-runs op main groen; open PR's #51 en #45 groen.
resultaat: Pagina in sync. Open: totalen (bonnen, exceptions, alerts, geparkeerd, cron-runs, ai_call, leads, ads) pas weer meetbaar na besluit 10; eerste contentmolen-ronde van 7 sep en migraties 039 en 042 niet te bevestigen zonder database.
regel: Een merge sluit een experiment-rij niet; wie deployt zet de rij op live met PR-nummer, anders leest de volgende sessie "wacht op deploy" bij iets dat al twaalf uur live is. En: de dagronde heeft een leesroute naar de database nodig die de security-gate niet omzeilt, een aparte read-only sleutel lokaal.
